...
if __name__ == "__main__":
    ...
    X = df.drop(columns=["duration_years", "event"]).select_dtypes(include=[np.number])
    ...
    shap_coxph(cph=cph, X=X_coxph, time_horizon=t_star)
    shap_rsf(rsf_model=rsf, X=X_rsf, time_horizon=t_star)
    shap_coxtime(model_ct=model_ct, scaler=scaler_ct, X=X_coxtime_input, time_horizon=t_star)
